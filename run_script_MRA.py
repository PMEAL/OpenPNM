...
"FOR IP"
#IP = OpenPNM.ALG.InvasionPercolationAlgorithmTiming(net=pn,loglevel=20,end_condition='breakthrough')
#IP.run()
...
#MT = OpenPNM.ALG.FicksLawDiffusion(pn,Alg='IP',Psequence=[max(pn.pore_properties['IP_Pseq'])/2])

"FOR OP"
OP = OpenPNM.ALG.OrdinaryPercolationAlgorithm(pn, npts=100, inv_faces=[1])
OP.run()
P = 0.04
...
#MT = OpenPNM.ALG.FicksLawDiffusion(pn,Alg='None',Pinvaded=Pores)
