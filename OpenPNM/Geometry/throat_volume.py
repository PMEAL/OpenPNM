...
def cylinder(geometry,
    ...
    """
    value=sp.pi/4*network.get_throat_data(prop=throat_length)*network.get_throat_data(prop=throat_diameter)**2
    network.set_throat_data(locations=geometry,prop=propname,data=value)
...
def cuboid(geometry,
    ...
    - note: this will need to account for volume taken up by spherical pore bodies
    """
    value=network.get_throat_data(prop=throat_length)*network.get_throat_data(prop=throat_diameter)**2
    network.set_throat_data(locations=geometry,prop=propname,data=value)