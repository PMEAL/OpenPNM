...
def Fuller_scaling(fluid,network,DABo=2.09e-5,To=298.,Po=101325.,**params):
    ...
    Pi = network.get_pore_data(fluid=fluid.name,prop='pressure')
...
def TynCalus(fluid,network,VA=0.018,VB=0.018,sigma_A=0.07197,sigma_B=0.07197,**params):
    ...
    """
    T = network.get_pore_data(fluid=fluid.name,prop='temperature')
    mu = network.get_pore_data(fluid=fluid.name,prop='viscosity')
...
def TynCalus_Scaling(fluid,network,DABo=2.09e-9,To=298.,mu_o=8.90e-4,**params):
    ...
    """
    Ti = network.get_pore_data(fluid=fluid.name,prop='temperature')
    mu_i = network.get_pore_data(fluid=fluid.name,prop='viscosity')
