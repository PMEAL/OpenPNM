...
class GenericLinearTransport(GenericAlgorithm):
    ...
    def setup(self, conductance, quantity, super_pore_conductance):
        ...
        """
        # Assigning conductance between each group of pores with Neumann_group BC and their corresponding super pore
        ...
        # Checking for the linear terms which might be added to the coeff diagonal or RHS
    ...
    def set_source_term(self, source_name=None, pores=None, x0=None, tol=None,
        ...
        if source_name is not None:
            ...
            for source_name in s_group:
                ...
                try:
                    ...
                    Exception('The attached phase in the algorithm ' +
                              self.name + ', does not have the source property ' +
                ...
                if mode == 'remove':
                            ...
                            if pores is 'all':
                                ...
                                for item in self.labels():
                                    if 'pore.source_' in item:
                                        ...
                                        for s in s_mode:
                                            try:
                                                self['pore.source_' + s + '_s1_' + prop][pores] = sp.nan
                                            except KeyError:
                                                pass
                                            try:
                                                self['pore.source_' + s + '_s2_' + prop][pores] = sp.nan
                                            except KeyError:
                                                pass
                        else:
                            raise Exception('No pores/source_name are sent to the set_term_method!')
                    else:
                        if pores is None:
                            try:
                                del self['pore.source_' + prop]
                    ...
                    if 'pore.source_' + prop not in self.labels() or mode == 'overwrite':
                        ...
                        self['pore.source_' + source_mode + '_s1_' + prop] = temp_arr
                        self['pore.source_' + source_mode + '_s2_' + prop] = temp_arr
            ...
            Exception('No source_name has been sent for set_source_term method in the algorithm ' + self.name)
    ...
    def solve(self, A=None, b=None, iterative_solver=None, **kwargs):
        ...
        self.X = X
        self._Neumann_super_X = self.X[~sp.in1d(sp.arange(0, self._coeff_dimension), self.Ps)]
    ...
    def _do_inner_iteration_stage(self, guess, **kwargs):
        ...
        for label in self.labels():
            if 'pore.source_' in label:
                ...
                if 'pore.source_nonlinear_s1_' + source_name in self.props():
                    ...
                    self.set_source_term(source_name=source_name, pores=self.pores(label),
                                         x0=guess, tol=tol, maxiter=maxiter, mode='update')
        ...
        A = self._build_coefficient_matrix(modified_diag_pores=pores,
                                           diag_added_data=S1, mode='modify_diagonal')
    ...
    def return_results(self, pores=None, throats=None, **kwargs):
        ...
        dx = sp.squeeze(sp.diff(self[self._quantity][self._net.find_connected_pores(self.Ts)],
                                n=1, axis=1))
    ...
    def _build_coefficient_matrix(self, modified_diag_pores=None, diag_added_data=None, mode='overwrite'):
        r"""
        ...
        if mode == 'overwrite':
            ...
            try:
                ...
                temp = self.Ps
                logger.warning('No direct Dirichlet boundary condition has been'
                               ' applied to the phase ' + self._phase.name +
                               ' in the algorithm ' + self.name)
            ...
            try:
                ...
                if l_g_super == 1:
                    self.super_pore_conductance = [sp.array(self.super_pore_conductance)] * self._extra_Neumann_size
        ...
        if mode in ['overwrite', 'modify_diagonal']:
            ...
            if modified_diag_pores is not None and diag_added_data is not None:
                if sp.size(modified_diag_pores) == sp.size(diag_added_data):
                    diagonal_vals[modified_diag_pores] = self._diagonal_vals[modified_diag_pores] + \
                                                         diag_added_data
                else:
                    raise Exception('Provided data and pores for modifying '
                                    'coefficient matrix should have the same size!')
                if mode == 'overwrite':
    ...
    def _calc_eff_prop(self, check_health=False):
        ...
        if check_health:
            ...
            PnI = self._net.find_neighbor_pores(pores=inlets, mode='not_intersection',
                                                excl_self=True)
            if sp.shape(PnI) != sp.shape(inlets):
                ...
                pass
            PnO = self._net.find_neighbor_pores(pores=outlets, mode='not_intersection',
                                                excl_self=True)
