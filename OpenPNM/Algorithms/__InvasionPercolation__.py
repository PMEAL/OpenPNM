...
class InvasionPercolation(GenericAlgorithm):
    ...
    def run(self,net,**params):
        ...
        net : OpenPNM.Network.GenericNetwork object
            pore network where invasion algorithm will take place
        inv_fluid : OpenPNM.Fluids.GenericFluid object
            invading fluid which will displace defending fluid
        def_fluid : OpenPNM.Fluids.GenericFluid obect
            defending fluid which will be displaced by invading fluid
    ...
    def _do_outer_iteration_stage(self):
        ...
        self._inv_fluid.pore_conditions['IP_inv_final']=np.array(self._Pinv,dtype=np.int)
        self._inv_fluid.pore_conditions['IP_inv_original']=np.array(self._Pinv_original,dtype=np.int)
        self._inv_fluid.throat_conditions['IP_inv']=np.array(self._Tinv,dtype=np.int)
        self._inv_fluid.pore_conditions['IP_inv_seq']=np.array(self._psequence,dtype=np.int)
        self._inv_fluid.throat_conditions['IP_inv_seq']=np.array(self._tsequence,dtype=np.int)
        if self._timing:
            self._inv_fluid.pore_conditions['IP_inv_time']=np.array(self._Ptime,dtype=np.float)
...
if __name__ =="__main__":
    ...
    ip_timing_params = {'inv_fluid':water,
                 'def_fluid':air,
                 'inlets':inlets,
                 'outlets':outlets,
                 'timing':'ON',
