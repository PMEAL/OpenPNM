...
def Washburn(net,sigma,theta):
    ...
    """
    return -4*sigma*_sp.cos(_sp.radians(theta))/net.throat_properties['diameter']
...
def Purcell(net,sigma,theta,r_toroid):
    ...
    Pc = (-2*sigma/r)*(_sp.cos(_sp.radians(theta - alpha))/(1 + R/r*(1-_sp.cos(_sp.radians(alpha)))))
    return Pc    
...
def Morrow(net,sigma,theta):
    ...
    """
    return -4*sigma*_sp.cos(_sp.radians(2/3*(180-theta)))/net.throat_properties['diameter']