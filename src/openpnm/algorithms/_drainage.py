...
@docstr.get_sections(base="DrainageSettings", sections=["Parameters"])
@docstr.dedent
    ...
    def apply_trapping(self):
        ...
        * ``'pore/throat.invasion_sequence'`` is set to ``0``
        ...
        for p in np.unique(pseq):
            s, b = site_percolation(conns=self.network.conns, occupied_sites=pseq > p)
            # Identify uninvaded throats between previously invaded pores within same cluster
            both_pores_invaded = (pseq[self.network.conns[:, 0]] <= p) & (
                pseq[self.network.conns[:, 1]] <= p
            )
...
if __name__ == "__main__":
    ...
    if 1:
        ...
        ax = op.topotools.plot_coordinates(
            pn, pores=p, s=500, color_by=drn["pore.invasion_pressure"]
        )
        ax = op.topotools.plot_connections(
            pn, throats=t, linewidth=5, color_by=drn["throat.invasion_pressure"], ax=ax
        )
    ...
    if 0:
        ...
        for j, i in enumerate(tqdm(np.unique(tseq)[:-1])):
            ...
            op.topotools.plot_coordinates(
                pn, drn["pore.trapped"], c="c", marker="o", markersize=100, ax=ax
            )
    ...
    if 0:
        ...
        ax = op.topotools.plot_coordinates(pn, pores=drn["pore.trapped"], color_by=s)
        ax = op.topotools.plot_coordinates(pn, pores=pseq <= p, c="k", ax=ax)
        ax = op.topotools.plot_connections(pn, throats=drn["throat.trapped"], color_by=b, ax=ax)
        t = drn["throat.invasion_pressure"] <= p
        ax = op.topotools.plot_connections(pn, throats=t, c="k", linestyle="--", ax=ax)
