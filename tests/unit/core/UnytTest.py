...
#         self.geo1 = op.geometry._StickAndBall(network=self.net,
#                                              pores=np.arange(0, 75),
#                                              throats=np.arange(0, 150))
#         self.geo2 = op.geometry._StickAndBall(network=self.net,
...
#         self.net['pore.test_prop'] = np.ones(self.net.Np) * unyt.m
#         t = self.net['pore.test_prop']
#         assert t.units == unyt.m

#     def test_set_and_get_on_phase(self):
#         self.phase['pore.test_prop2'] = np.ones(self.phase.Np) * unyt.m
...
#         self.geo1['pore.test_prop3'] = np.ones(self.geo1.Np) * unyt.m
#         self.geo2['pore.test_prop3'] = np.ones(self.geo2.Np) * unyt.m
#         t = self.net['pore.test_prop3']
#         assert t.units == unyt.m

#     def test_set_on_geo1_not_geo2_get_on_net(self):
#         self.geo1['pore.test_prop4'] = np.ones(self.geo1.Np) * unyt.m
#         t = self.net['pore.test_prop4']
#         assert t.units == unyt.m

#     def test_set_on_phys1_and_phys2_get_on_phase(self):
#         self.phys1['pore.test_prop5'] = np.ones(self.phys1.Np) * unyt.m
#         self.phys2['pore.test_prop5'] = np.ones(self.phys2.Np) * unyt.m
#         t = self.phase['pore.test_prop5']
#         assert t.units == unyt.m

#     def test_set_on_phys1_not_phys2_get_on_phase(self):
#         self.phys1['pore.test_prop6'] = np.ones(self.phys1.Np) * unyt.m
#         t = self.phase['pore.test_prop6']
#         assert t.units == unyt.m

#     def test_interleave_compatible_units(self):
#         self.phys1['pore.test_prop5'] = np.ones(self.phys1.Np) * unyt.m
#         self.phys2['pore.test_prop5'] = np.ones(self.phys2.Np) * unyt.cm
#         t = self.phase['pore.test_prop5']
#         assert t.units == unyt.m

#     def test_interleave_incompatible_units(self):
#         self.phys1['pore.test_prop5'] = np.ones(self.phys1.Np) * unyt.m
#         self.phys2['pore.test_prop5'] = np.ones(self.phys2.Np) * unyt.kg
