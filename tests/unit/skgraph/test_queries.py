...
class SKGRQueriesTest:
    ...
    def test_find_connected_nodes_undirected(self):
        g = cubic(shape=[3, 2, 1])
        c = queries.find_connected_nodes(g=g, inds=[0, 2, 4], logic='or')
        assert np.all(c == [0, 1, 3, 4, 5])
        c = queries.find_connected_nodes(g=g, inds=[0, 2, 4], logic='xor')
        assert np.all(c == [0, 3, 4, 5])
        c = queries.find_connected_nodes(g=g, inds=[0, 2, 4], logic='xnor')
        assert np.all(c == [1])
        c = queries.find_connected_nodes(g=g, inds=[0, 2, 4], logic='and')
        assert np.all(c == [])
    ...
    def test_find_connected_nodes_directed(self):
        ...
        with pytest.raises(Exception):
            _ = queries.find_connected_nodes(g=g, inds=[0, 2, 4])

    def test_find_neighbor_edges_undirected(self):
        g = cubic(shape=[3, 2, 1])
        c = queries.find_neighbor_edges(g=g, inds=[0, 2, 4], logic='or')
        assert np.all(c == [0, 1, 2, 3, 5])
        c = queries.find_neighbor_edges(g=g, inds=[0, 2, 4], logic='xor')
        assert np.all(c == [0, 1, 2])
        c = queries.find_neighbor_edges(g=g, inds=[0, 2, 4], logic='xnor')
        assert np.all(c == [3, 5])
        with pytest.raises(Exception):
            _ = queries.find_neighbor_edges(g=g, inds=[0, 2, 4], logic='and')

    def test_find_neighbor_edges_directed(self):
        g = cubic(shape=[3, 2, 1])
        g['edge.conns'][1, :] = [1, 0]
        c = queries.find_neighbor_edges(g=g, inds=[0, 1], logic='or')
        assert np.all(c == [0, 1, 3, 4])
        c = queries.find_neighbor_edges(g=g, inds=[0], logic='or')
        assert np.all(c == [0, 1, 3])
        c = queries.find_neighbor_edges(g=g, inds=[0, 1, 4], logic='xor')
        assert np.all(c == [2, 3, 4, 5])
        c = queries.find_neighbor_edges(g=g, inds=[0, 1, 4], logic='xnor')
        assert np.all(c == [0, 1])
        with pytest.raises(Exception):
            _ = queries.find_neighbor_edges(g=g, inds=[0, 2, 4], logic='and')
    ...
    def test_find_connecting_edges_undirected(self):
        g = cubic(shape=[3, 2, 1])
        c = queries.find_connecting_edges(inds=[0, 1], g=g)
        assert np.all(c == [0])
        c = queries.find_connecting_edges(inds=[[0, 1]], g=g)
        assert np.all(c == [0])
        c = queries.find_connecting_edges(inds=[[0, 1], [0, 2]], g=g)
        assert np.all(c == [0, 3])
        c = queries.find_connecting_edges(inds=[[0, 1], [0, 2], [0, 5]], g=g)
    ...
    def test_find_connecting_edges_directed(self):
        ...
        c = queries.find_connecting_edges(inds=[0, 1], g=g)
        assert np.all(c == [0])
        c = queries.find_connecting_edges(inds=[1, 0], g=g)
        assert np.all(c == [1])
        c = queries.find_connecting_edges(inds=[[0, 1], [1, 0]], g=g)
        assert np.all(c == [0, 1])
        c = queries.find_connecting_edges(inds=[[0, 1], [1, 0], [1, 5]], g=g)
    ...
    def test_find_common_edges_directed(self):
        ...
        g['edge.conns'][1, :] = [1, 0]
        with pytest.raises(Exception):
            _ = queries.find_common_edges(g=g, inds_1=[0, 1], inds_2=[4, 5])
    ...
    def test_find_complementary_edges_directed(self):
        g = cubic(shape=[3, 2, 1])
        g['edge.conns'][1, :] = [1, 0]
        c = queries.find_complementary_edges(inds=[0, 1, 2], g=g)
        assert np.all(c == np.array([3, 4, 5, 6]))
        c = queries.find_complementary_edges(inds=[0, 1, 2], g=g, asmask=True)
    ...
    def test_find_path_undirected(self):
        g = cubic(shape=[4, 4, 1])
        p = queries.find_path(g=g, pairs=[[0, 11], [11, 0]])
    ...
    def test_find_path_undirected_neighboring_pores(self):
        g = cubic(shape=[4, 4, 1])
        p = queries.find_path(g=g, pairs=[[0, 1], [1, 0]])
