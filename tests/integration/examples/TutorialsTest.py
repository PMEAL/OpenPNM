...
test_dir = os.path.join(examples_dir, 'tutorials')
...
class TutorialTest():
    ...
    def test_tutorial_01(self):
        nbook = os.path.join(test_dir, '01 - Intro to OpenPNM - Basics.ipynb')
        rc = self._notebook_run(nbook)
        assert rc

    def test_tutorial_02(self):
        nbook = os.path.join(test_dir, '02 - Intro to OpenPNM - Intermediate.ipynb')
        rc = self._notebook_run(nbook)
        assert rc

    def test_tutorial_03(self):
        nbook = os.path.join(test_dir, '03 - Intro to OpenPNM - Advanced.ipynb')
        rc = self._notebook_run(nbook)
        assert rc
