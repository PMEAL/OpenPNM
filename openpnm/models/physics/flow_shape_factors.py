...
def ball_and_stick(target, pore_area='pore.area',
    ...
    # warning to inform about the approximation
    if ((sum(D1<=2*L1)!=0) or (sum(D1<=2*L1)!=0)):
        logger.warning('D/L for some pores fall into conical frustum approximation')
    MLD1, MLD2 = [(Di<=2*Li) & mi for Di, Li, mi in zip([D1, D2], [L1, L2], [m1, m2])]
    #Integral for Dt!=0.5Di with conical_frustum_and_stick
