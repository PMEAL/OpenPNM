...
def mixed_multiphase_diffusion_MW(
    target,
    pore_area="pore.area",
    throat_area="throat.area",
    pore_diameter="pore.diameter",
   ...
   (Henry's partitioning law)
    ...
    Dt = phase.interpolate_data(propname=pore_diffusivity)[throats]
    # Calculating Knudsen diffusivity
    MWs=_np.ones(len(network.Ps))*phase[molecular_weight][network.pores(gas_pores)[0]]
    d1, d2 = network[pore_diameter][cn].T
    dt = network[throat_diameter][throats]
    MW1, MW2 = MWs[cn].T
    MWt = phase.interpolate_data(propname=molecular_weight)[throats]
...
def mixed_multiphase_diffusion_Kn_FD(
    target,
    pore_area="pore.area",
    throat_area="throat.area",
    pore_diameter="pore.diameter",
   ...
   (Henry's partitioning law)
    ...
    D2e = (1 / DK2 + 1 / D2) ** (-1)
    Dte = (1 / DKt + 1 / Dt) ** (-1)
    # Correct for the liquid_filled pores FD
    # Find g for half of pore 1, throat, and half of pore 2 + apply shape factors
    g1 = (D1e * A1) / L1 * SF1
    g2 = (D2e * A2) / L2 * SF2
    gt = (Dte * At) / Lt * SFt
