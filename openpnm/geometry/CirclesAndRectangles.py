...
class CirclesAndRectangles(GenericGeometry):
    r"""
    Circles and Rectangles subclass of GenericGeometry. This subclass is
    ...
    >>> from openpnm.geometry import CirclesAndRectangles
    >>> pn = op.network.CubicDual(shape=[5, 5, 5])
    >>> Ps = pn.pores('primary')
    >>> Ts = pn.throats('primary')
    >>> geo1 = CirclesAndRectangles(network=pn, pores=Ps, throats=Ts)
    >>> Ps = pn.pores('secondary')
    >>> Ts = pn.throats(['secondary', 'interconnect'])
    >>> geo2 = CirclesAndRectangles(network=pn, pores=Ps, throats=Ts)
    ...
    def __init__(self, **kwargs):
        ...
        self.add_model(propname='throat.diffusive_size_factors',
                       model=gmods.diffusive_size_factors.circles_and_rectangles,
                       pore_diameter="pore.diameter",
                       throat_diameter="throat.diameter")

        self.add_model(propname='throat.hydraulic_size_factors',
                       model=gmods.hydraulic_size_factors.circles_and_rectangles,
                       pore_diameter="pore.diameter",
                       throat_diameter="throat.diameter")
