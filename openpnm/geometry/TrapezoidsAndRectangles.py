...
class TrapezoidsAndRectangles(GenericGeometry):
    r"""
    Trapezoids and Rectangles subclass of GenericGeometry. This subclass is
    ...
    >>> geo1 = TrapezoidsAndRectangles(network=pn, pores=Ps, throats=Ts)
    >>> Ps = pn.pores('secondary')
    >>> Ts = pn.throats(['secondary', 'interconnect'])
    >>> geo2 = TrapezoidsAndRectangles(network=pn, pores=Ps, throats=Ts)
    ...
    def __init__(self, **kwargs):
        ...
        self.add_model(propname='pore.cross_sectional_area',
                       model=mods.geometry.pore_cross_sectional_area.trapezoid,
