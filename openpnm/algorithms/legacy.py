...
class TransientReactiveTransport(ReactiveTransport):
    ...
    def run(self, t=None, solver=None):
        r"""
        ...
        self._update_iterative_props()
        self._run_transient(t=t, solver=solver)

    def _run_transient(self, t, solver):
        """r
        ...
        if s == 'steady':
            ...
            self._t_run_reactive(solver=solver)
        # Time marching step
        else:
            ...
            for time in np.arange(t+dt, tf+dt, dt):
                ...
                self._b_t = self._b.copy()
                x_old = self[quantity]
                self._t_run_reactive(solver=solver, x0=x_old)
                x_new = self[quantity]
    ...
    def _t_run_reactive(self, solver, x0=None):
        """r
        ...
        max_it = int(self.settings['newton_maxiter'])
        ...
        for itr in range(max_it):
            ...
            res = np.linalg.norm(self._get_residual())
            ...
            xnew, exit_code = solver.solve(A=self.A, b=self.b, x0=x)
            x = xnew * w + x * (1 - w)
            self[quantity] = x
    ...
    def _is_converged(self, x=None):
        ...
        """
        res = np.linalg.norm(self._get_residual(x=x))
        ...
        # Check convergence
        tol = self.settings["f_rtol"]
        res_tol = np.linalg.norm(self.b) * tol
