...
#        plt.grid(True)
    ...
    def update_phase_and_phys(self, satnum, invnum, pore_occ, throat_occ):
        ...
        if self.settings['auto'] is not False:
            self.settings['auto_inv']['pore.occupancy'] = pore_occ[str(invnum)][satnum]
            self.settings['auto_def']['pore.occupancy'] = 1-pore_occ[str(invnum)][satnum]
            self.settings['auto_inv']['throat.occupancy'] = throat_occ[str(invnum)][satnum]
            self.settings['auto_def']['throat.occupancy'] = 1-throat_occ[str(invnum)][satnum]
...
#            ly = bmax-bmin
#            lz = cmax-cmin
#            defend_phase=self.settings['auto_def']
#            invade_phase=self.settings['auto_inv']
...
#        else:
#            defend_phase=self.project.phases(self.settings['def_phase'])
#            invade_phase=self.project.phases(self.settings['inv_phase'])
#        St_def = StokesFlow(network=network,
#                            phase=defend_phase)
#        St_def.setup(conductance='throat.hydraulic_conductance')
#        St_def.set_value_BC(pores=inlets, values=1)
#        St_def.set_value_BC(pores=outlets, values=0)
#        St_def.run()
#        if self.settings['user_inlets'] is not True:
#            print('not specified')
##            [da, dl]=self.domain_l_a()
...
#        St_inv.set_value_BC(pores=inlets, values=1)
#        St_inv.set_value_BC(pores=outlets, values=0)
#        St_inv.run()
#        if self.settings['user_inlets'] is not True:
#            print('not specified')
...
#            # def_p=self.project.phases(self.settings['def_phase'])
#            if self.settings['auto'] is not False:
#                def_tp=self.settings['auto_def']
#                inv_tp=self.settings['auto_inv']
#            else:
#                def_tp=self.project.phases(self.settings['def_phase'])
#                inv_tp=self.project.phases(self.settings['inv_phase'])
