...
class Base(dict):
    ...
    def props(self, element=None, mode='all'):
        ...
        if hasattr(self, 'models'):
            ...
            constants = [i for i in props if i not in models]
            if 'models' in mode:
                temp = [i for i in models if i.split('.')[0] in element]
                vals.extend(temp)
            if 'constants' in mode:
                temp = [i for i in constants if i.split('.')[0] in element]
                vals.extend(temp)
        return vals
    ...
    def _parse_indices(self, indices):
        ...
        if sp.issubdtype(locs.dtype, sp.number) or sp.issubdtype(locs.dtype, sp.bool_):
            pass
        else:
