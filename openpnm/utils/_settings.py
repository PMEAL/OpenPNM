...
class SingleSetting(dict):
    def __init__(self, *args):
        if len(args) == 0:
            super().__setitem__('value', None)
            self['dtype'] = ''
            self['desc'] = ''
        else:
            super().__init__(*args)
    ...
    def __setitem__(self, key, value):
        if key == 'value':
            if self['value'] is None:
                ...
                b = self['value'].__class__.__mro__
                ...
                if len(check) > 0:  # If they share comment parent class
                    ...
                    old = type(self['value'])
                    new = type(value)
                    raise TypeError("Attribute can only accept " +
...
class SettingsAttr3:
    ...
    def __getattr__(self, attr):
        return getattr(self, '_settings')[attr]['value']

    def __setattr__(self, attr, value):
        self._settings[attr]['value'] = value
    ...
    @property
    def __doc__(self):
        return redoc(self._settings)


s = SingleSetting({'value': 33, 'dtype': 'int', 'desc': 'a number'})
s2 = SingleSetting({'value': 3.3, 'dtype': 'float', 'desc': 'another number'})
d = SettingsAttr3()
d._settings.update({'foo': s})
d._settings.update({'bar': s2})
