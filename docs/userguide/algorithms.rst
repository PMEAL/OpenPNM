...
*Example for Applying boundary conditions*
...
Fickian
-------------------------------------------------------------------------------
OpenPNM applies Fick’s law to determine flux between pores. By using pipe network (or electrical resistor network) analogy, it will calculate the conductance of the conduit (a throat plus half of each adjoining pore) to determine mass rate between the pore and its neighboring pores. Diffusive conductance for a fluid in each conduit depends on the length and diameter of the conduit, the binary diffusion coefficient, the total molar concentration and also occupancy of the fluids in the bond.
Then Fick’s law for diffusion of fluid A through stagnant fluid B, can be used to determine mass rate between two neighboring pores. Applying species conservation equation to each pore in the network yields a sparse set of linear equations that can be solved with the appropriate boundary conditions to give mole fraction of the active fluid in each pore.
